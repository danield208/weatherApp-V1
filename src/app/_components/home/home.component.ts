...
export class HomeComponent implements OnInit {
	...
	checkForMobileWidth() {
		window.innerWidth < 1200 ? (this.showSidenav = false) : (this.showSidenav = true);
	}
	...
	checkGeoLoaded() {
		this.geoSubscription = this.geo.locationLoaded.subscribe((response: boolean): void => {
			if (response) this.geoLoaded = true;
		});
	}

	subscribeRouterEvents() {
		this.router.events.pipe(filter((e: Event): e is RouterEvent => e instanceof RouterEvent)).subscribe((res) => {
	...
	deleteItem() {
		...
		this.database.updateSavedCities({ savedcities: this.user.User.savedcities }).subscribe((res: any) => {
			// console.log(res);
		});
